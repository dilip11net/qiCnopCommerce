...
namespace Nop.Services.Catalog
{
    ...
    public partial class ProductService : IProductService
    {
        ...
        /// <summary>
        /// Gets a product by Api_itemId
        /// </summary>
        /// <param name="Api_itemId">SKU</param>
        /// <returns>Product</returns>
        public virtual Product GetProductByApi_itemId(int? api_itemId)
        {
            if ((api_itemId) == 0 || api_itemId == null)
                return null;

            var query = from p in _productRepository.Table
                        orderby p.Id
                        where !p.Deleted &&
                        p.Api_itemId == api_itemId
                        select p;
            var product = query.FirstOrDefault();
